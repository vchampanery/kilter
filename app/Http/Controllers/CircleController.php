...
class CircleController extends Controller
{
    ...
    {
        $userList = User::get(['id','name','city'])->toArray();
    ...
    public function store(Request $request)
    {
        ...
        $circleName['name'] = $params['name'];
        $circleName['created_by'] = $userId;

        $circleObj = circle::create($circleName);
        $rolename = circleRole::RoleId('ADMIN','circle');

        $circlemember['circle_id'] = $circleObj->id;
        $circlemember['role_id'] = $rolename->id;
        $circlemember['user_id'] = $userId;
        $circlemember['isActive'] = 1;
        $circleObj = circleMember::create($circlemember);

        foreach($params['member'] as $key=>$vlu){
            $circlemember = [];
            $circlemember['circle_id'] = $circleObj->id;
            $circlemember['user_id'] = $vlu;
            $rolename = circleRole::RoleId('MEMBER','circle');
            $circlemember['role_id'] = $rolename->id;
            $circlemember['isActive'] = 1;
            $circleObj = circleMember::create($circlemember);
        }
